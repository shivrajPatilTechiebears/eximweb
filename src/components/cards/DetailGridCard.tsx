...
export function DetailGridCard({ items, className }: DetailGridCardProps) {
  return (
    <section
      className={`bg-surface-container-lowest border border-outline-variant/70 rounded-lg p-4 shadow-sm${className ? ` ${className}` : ""}`}
    >
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-x-6 gap-y-2">
        {items.map((item) => (
          <div
            ...
            className={
              item.wide
                ? "md:col-span-2 xl:col-span-4 flex flex-col mt-1 pt-2 border-t border-outline-variant/30"
                : "flex flex-col"
            }
          >
