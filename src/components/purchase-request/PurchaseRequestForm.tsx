...
import {
  MOCK_PURCHASE_REQUESTS, STATUS_STYLE,
  PR_TYPE_OPTIONS, CURRENCY_OPTIONS, SHIPMENT_TERMS_OPTIONS,
  PAYMENT_TERMS_OPTIONS, TRANSPORTER_OPTIONS,
} from "./types";
import type { RequestDetailsFields, PurchaseItem, ScheduleRow } from "./types";
...
const DEFAULT_REQUEST_DETAILS: RequestDetailsFields = {
  prType: "", currency: "", shipmentTerms: "", paymentTerms: "",
...
export function PurchaseRequestForm({ mode, requestId }: PurchaseRequestFormProps) {
  const isCreate = mode === "create";
  const isView = mode === "view";
  const isEdit = mode === "edit";
  const disabled = isView;

  const record = requestId ? MOCK_PURCHASE_REQUESTS[requestId] : undefined;

  const [isGalleryOpen, setIsGalleryOpen] = useState(false);
  const [selectedItemId, setSelectedItemId] = useState<number | null>(null);
  const [requestDetails, setRequestDetails] = useState<RequestDetailsFields>({ ...DEFAULT_REQUEST_DETAILS, ...record?.requestDetails });
  ...
  const handleRequestDetailsChange = useCallback(<K extends keyof RequestDetailsFields>(key: K, value: RequestDetailsFields[K]) => {
    setRequestDetails((p) => ({ ...p, [key]: value }));
  }, []);
  ...
  const steps = [
    { label: "Request Details", icon: "receipt_long", complete: Object.values(requestDetails).every(Boolean) },
    {
  ...
  const prNumber = record?.prNumber ?? "PR-9284";
    ...
    <div className="flex-1 flex flex-col antialiased text-slate-800 bg-transparent">
      ...
      <DashboardPageHeader
        ...
        rightContent={
          isView ? (
            ...
            <StatusBadge label={isCreate ? `Draft ${prNumber}` : "Editing"} color="warning" pulse />
          )
        }
      />
      ...
      <main className="flex-1 px-6 py-4 pb-20 space-y-3">
        ...
        <Card className="card-glass">
          <CardHeader title="Request Details" hint={disabled ? undefined : "Tab · Enter to move between fields"} />
          <div className="px-5 py-4 grid grid-cols-4 gap-x-4 gap-y-3">
            <FormCombobox
              disabled={disabled} label="PR Type"
              value={requestDetails.prType}
              onChange={(v) => handleRequestDetailsChange("prType", v)}
              options={PR_TYPE_OPTIONS}
            />
            <FormInput
              disabled={disabled} label="Truck No"
              value={requestDetails.truckNo}
              onChange={(v) => handleRequestDetailsChange("truckNo", v)}
              placeholder="MH-12-AQ-9082"
            />
            <FormInput
              disabled={disabled} label="Driver Details"
              value={requestDetails.driverDetails}
              onChange={(v) => handleRequestDetailsChange("driverDetails", v)}
              placeholder="Ramesh Kumar (+91 98...)"
            />
            <FormInput
              disabled={disabled} label="Delivery Location"
              value={requestDetails.deliveryLocation}
              onChange={(v) => handleRequestDetailsChange("deliveryLocation", v)}
              placeholder="Mumbai Port Terminal 2"
            />
            <FormCombobox
              disabled={disabled} label="Currency"
              value={requestDetails.currency}
              onChange={(v) => handleRequestDetailsChange("currency", v)}
              options={CURRENCY_OPTIONS}
            />
            <FormCombobox
              disabled={disabled} label="Shipment Terms"
              value={requestDetails.shipmentTerms}
              onChange={(v) => handleRequestDetailsChange("shipmentTerms", v)}
              options={SHIPMENT_TERMS_OPTIONS}
            />
            <FormCombobox
              disabled={disabled} label="Payment Terms"
              value={requestDetails.paymentTerms}
              onChange={(v) => handleRequestDetailsChange("paymentTerms", v)}
              options={PAYMENT_TERMS_OPTIONS}
            />
            <FormCombobox
              disabled={disabled} label="Transporter"
              value={requestDetails.transporter}
              onChange={(v) => handleRequestDetailsChange("transporter", v)}
      ...
      <StickyFooter
        ...
        actions={
          isCreate ? (
            <>
              <ButtonLink href="/purchase-request" variant="cta-ghost">Cancel</ButtonLink>
              <Button variant="cta-secondary" icon="save">Save as Draft</Button>
              <Button variant="cta-sunset" icon="check">Submit PR</Button>
            </>
          ) : isEdit ? (
            <>
              <ButtonLink href={`/purchase-request/${requestId}`} variant="cta-ghost">Cancel</ButtonLink>
              <Button variant="pill-secondary">Save Changes</Button>
              <Button variant="cta-sunset" icon="check">Submit PR</Button>
            </>
          ) : (
            <>
              <ButtonLink href="/purchase-request" variant="pill-ghost">Back to List</ButtonLink>
              <ButtonLink href={`/purchase-request/${requestId}/edit`} variant="cta-sunset">Edit PR</ButtonLink>
            </>
          )
        }
      />
